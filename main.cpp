...
//Is the mouse drag creating or destroying cells?
unsigned int dragState = 0;
//Is the simulation running?
bool isStepping = 0;
//Used to toggle boards for simulation input
unsigned char activeBoard = 0;
//Keeps track of display mode
...
// Basic Game of Life rules variables
...
//Initial window and game parameters
int windowWidth = 600;
...
//functions for torus and sphere parametrization
float f(int i, int j)
{
	if(mode==2)
...
//fills vertex array with coordinates appropriate for the current 3d model
...
void keyInput(unsigned char key, int x, int y)
{
	switch(key) 
	{
		...
		//Increase number of squares horizontally
		case 'x':
			if (xSquares < X_MAX)
			{
				xSquares+=2;
			...
			break;
		//Decrease number of square horizontally
		case 'X':
			if (xSquares > 3)
			{
				xSquares-=2;
			...
			break;
		//Increase number of squares vertically
		case 'y':
			if (ySquares < Y_MAX)
			{
				ySquares++;
			...
			break;
		//Decrease number of square vertically
		case 'Y':
			if (ySquares > 3)
			{
				ySquares--;
		...
		case 'n':
			...
			glutPostRedisplay();
			break;
		//Decrease resolution of texture
		case 'R':
			TEX_SQUARE_WIDTH = TEX_SQUARE_WIDTH >> 1;
			if(!TEX_SQUARE_WIDTH)
				TEX_SQUARE_WIDTH = 1u;
			break;
		//Increase resolution of texture
		case 'r':
			TEX_SQUARE_WIDTH = TEX_SQUARE_WIDTH << 1;
			if(!TEX_SQUARE_WIDTH)
				TEX_SQUARE_WIDTH = 1u << sizeof(TEX_SQUARE_WIDTH);
			break;
		//Switch between Linear and Nearest Neighbor sampling
		case 't':
			...
			break;
		//Increase number of faces vertically
		case 'p':
			p += 3;
			if(p<0)
				p=INT_MAX;
			break;
		//Decrease number of faces vertically
		case 'P':
			p -= 3;
			if(p<=0)
				p=3;
			break;
		//Increase number of faces horizontally
		case 'q':
			q += 3;
			if(q<0)
				q=INT_MAX;
			break;
		//Decrease number of faces horizontally
		case 'Q':
			q -= 3;
			if(q<=0)
				q=3;
...
//Mouse Callback for click and drag
